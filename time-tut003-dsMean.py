...
dsMean = ds.meanVar(ds.get().find('eta'));

import os
os.chdir(os.environ['B2DXFITTERSROOT'] + "/tutorial");
if not os.path.exists('dsMean'):
	os.mkdir('dsMean')

from ROOT import TFile
g = TFile('dsMean/dsMean_%04d.root' % SEED, 'recreate')
g.WriteTObject(dsMean, 'dsMean/dsMean003_%04d' % SEED)
g.Close()
del g
